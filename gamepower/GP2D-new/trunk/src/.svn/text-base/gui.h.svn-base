...
public:
    ...
    * 1.  Draws the Widget, using GPSprite's draw() function.
    ...
    * it also creates the GPSprite object.  The texture can be any
    ...
    * only changes the Widget's pointer to a GPTexture.  This is the
    * best way to change an image on a mouseOver event or similar.  This
    * function does not read any files.
    *
    * @param newTexture The new GPTexture to be used as the image.
    */
    void setTexture(GPTexture newTexture);

    /**
    * registerMouseDown() registers an event that will be called on when
    * the left (?) mouse button is pressed down.
    *
    * @param button The button from GPMouseEvent::GPMouseButton enumeration
    *   which should be registered.
    * @param callbackFunction The function callback of prototype
    *   `void myFunc(GPMouseEvent&)`.
    */
    void registerMouseDown(GPMouseEvent::GPMouseButton button, void (*callbackFunction)(GPMouseEvent&));
    ...
    *   `void myFunc(GPMouseEvent&)`.
    */
    void registerMouseUp(GPMouseEvent::GPMouseButton button, void (*callbackFunction)(GPMouseEvent&));
    ...
    */
    ...
    /**
    * pollEvents calls on this Widget's GPInputHandler to poll mouse
...
public:
    ...
    *   `void myFunc(GPMouseEvent&)`.
    */
    virtual void registerMouseButtonDown(int uid, GPMouseEvent::GPMouseButton button, void (*callbackFunction)(GPMouseEvent&));
    virtual void registerMouseButtonUp(int uid, GPMouseEvent::GPMouseButton button, void (*callbackFunction)(GPMouseEvent&));
