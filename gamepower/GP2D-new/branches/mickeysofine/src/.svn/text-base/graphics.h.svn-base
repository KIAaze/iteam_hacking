...
public:
     ...
     * Gets the current X coord.
     */
    void SetX(float a_X) { m_x = a_X; }

    /**
     * Gets the current Y coord.
     */
    void SetY(float a_Y) { m_y = a_Y; }

    /**
     * Gets the current width.
     */
    void SetWidth(float a_Width) { m_w = a_Width; }

    /**
     * Gets the current height.
     */
    void SetHeight(float a_Height) { m_h = a_Height; }
...
protected:
    ...
    /**
     * A private (inline) function, which should return the alpha value from the current
     * surface. This function is only used by GP2DTexture::computeEdges(). TODO: documentation!!
     */
    Uint8 getAlpha(int y, int x);
...
public:
    ...
    /**
     * Loads a SDL_Suface off a filename. loads and turns it into a OpenGL texture in
     * video memory with the option to keep the orginal Surface
     *
     * @param file The filename to load a texture for.
     * @param a_KeepData Flag to store SDL_Surface true = keep it false = delete
     */
    bool Load(std::string file, bool a_KeepData);
    ...
    /**
     * The computeEdges function computes the edges in alpha images. It creates tiny bounding boxes.
     * TODO: lots more documentation, oh and put this out of the texture class for crying out loud.
     */
    void computeEdges();
...
public:
    ...
    /**
     * Creates a new GP2DSprite using the given texture.
     *
