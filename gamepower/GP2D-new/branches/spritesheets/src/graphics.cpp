...
void GP2DSprite::createAnimation(int width, int height) {
	m_anim_w = width;
	m_anim_h = height;
	m_anim_frame = 1;
	m_anim_frames = m_anim_w * m_anim_h;
...
void GP2DSprite::setFrame(int frame) {
	if (frame<1){ frame=1; }
	m_anim_frame = frame;
}

void GP2DSprite::setNextFrame() {
	m_anim_frame = m_anim_frame + 1;
	if(m_anim_frame > m_anim_frames){
		if(m_anim_loop == true){
			m_anim_frame = 1;
		} else {
			m_anim_frame = m_anim_frames;
		}
	}
}

void GP2DSprite::setLoop(bool val) {
	m_anim_loop = val;
}
        ...
        glBegin(GL_QUADS);
             ...
             */
			   ...
			   GLfloat x1,y1,x2,y2;
				...
				frame_wGL = ( frame_w / (GLfloat) m_texture->getSurface()->w ) + m_texture->getTextureOffsetX();
				frame_hGL = ( frame_h / (GLfloat) m_texture->getSurface()->h ) + m_texture->getTextureOffsetY();
				frame_xGL = ( ((m_anim_frame-1) % m_anim_w) * frame_wGL      ) + m_texture->getTextureRepeatX();
				frame_yGL = ( ((m_anim_frame-1) / m_anim_h) * frame_hGL      ) + m_texture->getTextureRepeatY();

				x1 = frame_xGL;
				y1 = frame_yGL;
				x2 = frame_xGL + frame_wGL;
				y2 = frame_yGL + frame_hGL;
