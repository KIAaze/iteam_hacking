...
*/
	...
	short int CollisionOnPixel(gp2d::Sprite * object1, int img1, Uint32 x, Uint32 y)
	{
 		GLfloat left1, left2, over_left;
		GLfloat right1, right2, over_right;
		GLfloat top1, top2, over_top;
		...
		// x-=(object1->width[img1]/2);
		
		SDL_LockSurface(surface1);
		SDL_GetRGBA(GetPixel(surface1,x,y),surface1->format,&pr,&pg,&pb,&pa2);	// Get the pixel's alpha from the second image.
		
/*
			DEBUG INFO: Remove this comment and recompile the library for showing a red area marking
			            the collision area found in the destination sprite.
*/						
		for(px = x-50; px <= x+50; px++){
		for(py = y-50; py <= y+50; py++){
			gp2d::DisableTexturing();
			gp2d::SetPointSize(2.0f);
			gp2d::BeginPoints();
				SetColor3(1,0,0); gp2d::DrawPixel(px,py);
			gp2d::EndPoints();
			gp2d::EnableTexturing();
		}
		}
		...
		if(pa2>0){ SDL_UnlockSurface(surface1); return(1); }
