...
namespace gp2d {
	...
	class GuiManager {
		...
		public:
			...
			// *********************************************************************
			// ******* Generaic callback function for event handler ****************
			// *********************************************************************
		...
		private:
			...
			// These are 2 dimensional arrays of key events
			// Each widget can register a key event (one event per key)
			// These events are stored as a list so all callback functions are
			// called at the same time. This enables multiple widgets to be called
			// based on a key press.
#if 0
			static map<int, void (*)(void)>		MouseLeftDownEvents;
			static map<int, void (*)(void)>		MouseLeftUpEvents;
			static map<int, void (*)(void)>		MouseRightDownEvents;
			static map<int, void (*)(void)>		MouseRightUpEvents;
#endif
			// all base widgets are located in the gui manager
			static vector<Widget*>		Canvases;	//These act like the canvas for the screen on which all other widgets can be placed
	};
