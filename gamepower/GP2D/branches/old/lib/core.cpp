...
// TODO: Pass the "VIDEO: Checking mode %i x %i @ %i bpp",width,height,depth" to PrintDebug
	...
	{
		PrintDebug ( "Texture: special constructor called -> filename=%s",filename );
		id=0;
		width=0.0;
		height=0.0;
		imgF = NULL;
	...
	{
		strcpy ( TextureName, filename );
		PrintDebug ( "[Texture::Load] called for %s",filename );
		//--------------------------------------------------
		//file->SDL_Surface
		...
		OriginalWidth=imgFile->w;
		OriginalHeight=imgFile->h;
		imgFile=resize2 ( imgFile,texSize );
//		SDL_SaveBMP ( imgFile,"imgFile.bmp" );
//		exit(7);
...
//exit(7);
	...
	*/
	Uint32 GetPixel ( SDL_Surface *surface, Uint32 x, Uint32 y )
	{
		...
		if ( x < 0 ) { PrintDebug ( "<ERROR> GetPixel -> X less than zero! (x<0)." ); exit(67); }
		if ( y < 0 ) { PrintDebug ( "<ERROR> GetPixel -> Y less than zero! (y<0)." ); exit(67); }
		if ( x >= ( Uint32 ) surface->w ) { PrintDebug ( "<ERROR> GetPixel -> X greather than surface's width! (x>=surface->w)." ); exit(67); }
		if ( y >= ( Uint32 ) surface->h ) { PrintDebug ( "<ERROR> GetPixel -> Y greather than surface's height! (y>=surface->h)." ); exit(67); }
...
//----------------------
	...
	//resizes to a power of 2 texture using zoomSurface
	//problem: sometimes the resizing isn't good enough and you get 511 instead of 512 for example
	SDL_Surface* resize ( SDL_Surface* image, int texSize )
	{
		...
		if ( sf_h!=1 || sf_w!=1 )
		{
			...
			PrintDebug ( "[resize] texture resized to (%dx%d)",ret->w,ret->h );
			if ( ( sf_w*image->w ) !=ret->w || ( sf_h*image->h ) !=ret->h )
			{
	...
	int wanted_size ( int texSize, int x_current )
	{
		int x_wanted=closest_power_of_two ( x_current );
		if ( x_wanted < x_current ) x_wanted = 2*x_wanted;
		if ( x_wanted > texSize ) x_wanted = texSize;
		return ( x_wanted );
	}
...
// 		SDL_Surface *ret;
		...
		if ( new_h!=image->h || new_w!=image->w )
		{
			...
			SDL_Surface* ret = SDL_CreateRGBSurface ( image->flags, new_w, new_h, 32, rmask, gmask, bmask, amask );

			//Convert the surface to the same format as image
			SDL_ConvertSurface ( ret, image->format, image->flags );
			...
			//SDL_SaveBMP(ret,"ret.bmp");//exit(45);
			//if you know how to make this work with SDL_BlitSurface, then please do! ;)
	...
	//special thanks to  Sabbac from the freegamedev forums. This function worked without any modifications. :)
	...
	void br_blit ( SDL_Surface* src, SDL_Rect sr, SDL_Surface* dst )
	{
		...
		for ( int y=0; y<src->h; y++ )
		{
			...
			memcpy ( destP, srcP, src->w*4 /*bytesToMove*/ );
		}
	}
	...
	bool SDL_OGL_SaveScreenshot ( const char *FileName )
	{
		...
		SDL_Surface* temp=SDL_DisplayFormatAlpha ( Screen );
