...
// Calculate the bounding box of each sprite's chosen image.
		...
		left1 = object1->GetX()- ( object1->width[imgid1]/2 );// Obj1's top-left X minus (Obj1's width / 2)
		top1 = object1->GetY()- ( object1->height[imgid1]/2 );// Obj1's top-left Y minus (Obj1's height / 2)
		right1 = object1->GetX() + object1->width[imgid1]- ( object1->width[imgid1]/2 );// Obj1's bottom-right X plus Obj1's width minus (Obj1's width / 2)
		bottom1 = object1->GetY() + object1->height[imgid1]- ( object1->height[imgid1]/2 );// Obj1's bottom-right Y plys Obj1's height minus (Obj1's height / 2)

		left2 = object2->GetX()- ( object2->width[imgid2]/2 );// Obj2's top-left X minus (Obj2's width / 2)
		top2 = object2->GetY()- ( object2->height[imgid2]/2 );// Obj2's top-left Y minus (Obj2's height / 2)
		right2 = object2->GetX() + object2->width[imgid2]- ( object2->width[imgid2]/2 );// Obj2's bottom-right X plus Obj2's width minus (Obj2's width / 2)
		bottom2 = object2->GetY() + object2->height[imgid2]- ( object2->height[imgid2]/2 );// Obj2's bottom-right Y plys Obj2's height minus (Obj2's height / 2)
...
// Calculate the collision region for each sprite, taking in account the collision bounding box.
		...
		left1 =  object1->GetX()- ( object1->width[img1]/2 ) + object1->col_x[img1];// Obj1's top-left X minus (Obj1's width / 2) plus Collision Rectangle's top-left X corner.
		top1 =  object1->GetY()- ( object1->height[img1]/2 ) + object1->col_y[img1];// Obj1's top-left Y minus (Obj1's height / 2) plus Collision Rectangle's top-left Y corner.
		right1 = object1->GetX()- ( object1->width[img1]/2 ) + ( object1->col_x[img1] + object1->col_width[img1] );// Obj1's bottom-right X minus (Obj1's width / 2) plus (Collision Rectangle's bottom-right X corner plus Collision Rectangle's width).
		bottom1 = object1->GetY()- ( object1->height[img1]/2 ) + ( object1->col_y[img1] + object1->col_height[img1] );// Obj1's bottom-right Y minus (Obj1's height / 2) plus (Collision Rectangle's bottom-right Y corner plus Collision Rectangle's height).

		left2 =  object2->GetX()- ( object2->width[img2]/2 ) + object2->col_x[img2];// Obj2's top-left X minus (Obj2's width / 2) plus Collision Rectangle's top-left X corner.
		top2 =  object2->GetY()- ( object2->height[img2]/2 ) + object2->col_y[img2];// Obj2's top-left Y minus (Obj2's height / 2) plus Collision Rectangle's top-left Y corner.
		right2 = object2->GetX()- ( object2->width[img2]/2 ) + ( object2->col_x[img2] + object2->col_width[img2] );// Obj2's bottom-right X minus (Obj2's width / 2) plus (Collision Rectangle's bottom-right X corner plus Collision Rectangle's width).
		bottom2 = object2->GetY()- ( object2->height[img2]/2 ) + ( object2->col_y[img2] + object2->col_height[img2] );// Obj2's bottom-right Y minus (Obj2's height / 2) plus (Collision Rectangle's bottom-right Y corner plus Collision Rectangle's height).
...
// the sprite's position on the screen.
		...
		right_off  = ( over_right - ( object2->GetX()- ( object2->width[img2]/2 ) ) );
		top_off    = ( over_top - ( object2->GetY()- ( object2->height[img2]/2 ) ) );
		left_off   = ( over_left - ( object2->GetX()- ( object2->width[img2]/2 ) ) );
		bottom_off = ( over_bottom - ( object2->GetY()- ( object2->height[img2]/2 ) ) );
...
//added by KIAaze
//TODO: Test the collision sytem extensively. Make sure it's really pixel-perfect, even for animated sprites, image by image. Add clear documentation on how it works. Synchronize with GP2D-new.
...
// GLfloat over_width, over_height;
		...
		SDL_Surface *surface1, *surface2;
		if(object1->animationSheet.size()==0) {printf("[CollisionPixel_2]FATAL ERROR: object1->animationSheet.size()==0\n");exit(12);};
		if(object2->animationSheet.size()==0) {printf("[CollisionPixel_2]FATAL ERROR: object2->animationSheet.size()==0\n");exit(12);};
		surface1 = object1->animationSheet[img1].spriteTexture->imgF;
		surface2 = object2->animationSheet[img2].spriteTexture->imgF;

// Alright... this is tricky. First, calculate the collision region just like in the function above.
		left1 =  object1->GetX()- ( object1->width[img1]/2 ) + object1->col_x[img1];
		left2 =  object2->GetX()- ( object2->width[img2]/2 ) + object2->col_x[img2];
		right1 = object1->GetX()- ( object1->width[img1]/2 ) + ( object1->col_x[img1] + object1->col_width[img1] );
		right2 = object2->GetX()- ( object2->width[img2]/2 ) + ( object2->col_x[img2] + object2->col_width[img2] );

		top1 =  object1->GetY()- ( object1->height[img1]/2 ) + object1->col_y[img1];
		top2 =  object2->GetY()- ( object2->height[img2]/2 ) + object2->col_y[img2];
		bottom1 = object1->GetY()- ( object1->height[img1]/2 ) + ( object1->col_y[img1] + object1->col_height[img1] );
		bottom2 = object2->GetY()- ( object2->height[img2]/2 ) + ( object2->col_y[img2] + object2->col_height[img2] );
...
//---------------------------------------------------
//TODO: finish those functions. Use a switch case or if statements eventually to have only one overlap function (same for collision_normal functions).
//TODO: Collision functions could be a member of the Sprite (or physObj) class: sprite1.collision(sprite2)
//---------------------------------------------------
	//returns the number of pixels overlapping with object1 stationary and object2 at location x and y
	int box_box_overlap(Sprite* object1,Sprite* object2,GLfloat x,GLfloat y)
	{return(0);}
	int box_pixel_overlap(Sprite* object1,Sprite* object2,GLfloat x,GLfloat y)
	{return(0);}
	int pixel_box_overlap(Sprite* object1,Sprite* object2,GLfloat x,GLfloat y)
	{return(0);}
	int pixel_pixel_overlap(Sprite* object1,Sprite* object2,GLfloat x,GLfloat y)
	{return(0);}
//---------------------------------------------------
	//returns the collision normal and tangent
	int box_box_collision_normal(Sprite* object1,Sprite* object2,GLfloat &Nx, GLfloat &Ny, GLfloat &Tx, GLfloat &Ty)
	{
		GLfloat x=object2->GetX();
		GLfloat y=object2->GetY();
		//calculate normal (force)
		Nx=box_box_overlap(object1,object2,x+1,y)-box_box_overlap(object1,object2,x-1,y);
		Ny=box_box_overlap(object1,object2,x,y+1)-box_box_overlap(object1,object2,x,y-1);
		//calculate tangent (slope)
		Tx=-Ny;
		Ty=Nx;
		return(0);
	}
	int box_pixel_collision_normal(Sprite* object1,Sprite* object2,GLfloat &Nx, GLfloat &Ny, GLfloat &Tx, GLfloat &Ty)
	{
		GLfloat x=object2->GetX();
		GLfloat y=object2->GetY();
		//calculate normal (force)
		Nx=box_pixel_overlap(object1,object2,x+1,y)-box_pixel_overlap(object1,object2,x-1,y);
		Ny=box_pixel_overlap(object1,object2,x,y+1)-box_pixel_overlap(object1,object2,x,y-1);
		//calculate tangent (slope)
		Tx=-Ny;
		Ty=Nx;
		return(0);
	}
	int pixel_box_collision_normal(Sprite* object1,Sprite* object2,GLfloat &Nx, GLfloat &Ny, GLfloat &Tx, GLfloat &Ty)
	{
		GLfloat x=object2->GetX();
		GLfloat y=object2->GetY();
		//calculate normal (force)
		Nx=pixel_box_overlap(object1,object2,x+1,y)-pixel_box_overlap(object1,object2,x-1,y);
		Ny=pixel_box_overlap(object1,object2,x,y+1)-pixel_box_overlap(object1,object2,x,y-1);
		//calculate tangent (slope)
		Tx=-Ny;
		Ty=Nx;
		return(0);
	}
	int pixel_pixel_collision_normal(Sprite* object1,Sprite* object2,GLfloat &Nx, GLfloat &Ny, GLfloat &Tx, GLfloat &Ty)
	{
		GLfloat x=object2->GetX();
		GLfloat y=object2->GetY();
		//calculate normal (force)
		Nx=pixel_pixel_overlap(object1,object2,x+1,y)-pixel_pixel_overlap(object1,object2,x-1,y);
		Ny=pixel_pixel_overlap(object1,object2,x,y+1)-pixel_pixel_overlap(object1,object2,x,y-1);
		//calculate tangent (slope)
		Tx=-Ny;
		Ty=Nx;
		return(0);
