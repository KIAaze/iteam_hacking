...
// 			if ( animationSheet[currentSheet].setNextFrame() == animationSheet[currentSheet].getspriteTotalFrames()-1 ) {
// 				// We have performed a full loop
// 				if ( iterateSheets == true ) {
// 					if ( selectNextSheet() == animationSheet.size()-1 ) {
// 						// We have performed a full loop
// 						return false;
// 					}
// 				}
...
// Set the window onto the texture based on texture coords 0->1
	...
	void Sprite::DrawFrame() {
		...
		if ( glIsTexture ( animationSheet[currentSheet].spriteTexture->id ) ) {
			//PrintDebug("[Sprite::DrawFrame] Youpi! It's a texture! id=%i",animationSheet[currentSheet].spriteTexture.id);
		}
		else {
		...
		glRotatef ( 180,0,0,1 );// Rotate 180° in Z

		if ( Mirror==true ) { glRotatef ( 180,0,1,0 ); }// We're mirrored? rotate 180° in Y
		if ( Flip==true ) { glRotatef ( 180,1,0,0 ); }// We're flipped? rotate 180° in X
...
//PrintDebug("Drawing %s",animationSheet[currentSheet].spriteTexture.TextureName);//)
		...
		if ( Mirror==true ) { glRotatef ( 180,0,1,0 ); }// We're mirrored? rotate 180ÃÂº in Y
		if ( Flip==true ) { glRotatef ( 180,1,0,0 ); }// We're flipped? rotate 180ÃÂº in X
...
// ---
	...
	bool Sprite::LoopFinished() {
		if ( !Backwards )
			...
			return ( currentSheet == animationSheet.size()-1 );
	}
