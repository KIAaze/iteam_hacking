...
// 		}
	...
	int Init() {
		...
		gp2d::SetDebug ( false );//set to false to remove all PrintDebug statements
		gp2d::Init();
	...
	int LoadGameData() {
		...
		Weapon_Texture.push_back ( new Texture ( "./data/weapons/pistol.png" ) );
...
// 		lines
	...
	void DrawGUI() {
		...
		if ( GameResW==800 ) {
			...
			Text ( iteam::Font[1], 155-fontwidths,600-45, 0,1,1,1, 1,1,1, "%i%c", ( unsigned int ) Player[CurPlayer].Angle, 0xB0 );
			Text ( iteam::Font[1], 230,600-65, 0,1,1,1, 1,1,1, "%i", ( int ) Player[CurPlayer].Power );
		}
		UpdateWeaponThumbs();
		Cam.Begin();
		AnglePointer.Move ( InGameGUI[0].x - ( VIEWPORT_WIDTH/2 ) + 161, InGameGUI[0].y + 50 );
	...
	void NextTurn() {
		...
		do {CurPlayer=(CurPlayer+1)%Player.size();} while(!Player[CurPlayer].Alive);//skips dead players
