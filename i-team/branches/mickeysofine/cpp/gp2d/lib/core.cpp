...
namespace gp2d {
	...
	void Texture::Load(const char filename[]){
		...
		if(width > texSize || height > texSize){
			...
			if (width <= texSize)
			{
				tmp = 1 << (int)floor((log((double)width)/log(2.0f)) + 0.5f);
			} else {
				tmp = texSize;
			}
			neww = tmp/width;
 			if (height <= texSize)
			{
				tmp = 1 << (int)floor((log((double)height)/log(2.0f)) + 0.5f); 
			} else {
				tmp = texSize;
			}
			newh = tmp/height;
			
			PrintDebug("TEXTURE: <WARNING> Texture size is maximum than the allowed by the video card (%ix%i). Resizing to %f,%f",texSize,texSize,neww,newh);
			
			SDL_Surface * newSurface;
			newSurface = zoomSurface(imgFile,neww,newh,SMOOTHING_ON);
			imgFile = newSurface;
		}
		...
		PrintDebug("TEXTURE: %s loaded. (%ix%i)",filename,imgFile->w,imgFile->h);		
	}
	...
	*/
	...
	bool SetPixelAlpha(SDL_Surface *surface, Uint32 x, Uint32 y)
	{
		...
		if( x > (Uint32) surface->w ){ PrintDebug("<ERROR> SetPixelAlpha -> X greather than surface's width! (x>surface->w)."); }
		if( y > (Uint32) surface->h ){ PrintDebug("<ERROR> SetPixelAlpha -> Y greather than surface's height! (y>surface->h)."); }
