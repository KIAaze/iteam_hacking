...
void GP2DWindow::createWindow(int width, int height, int bpp) throw(GP2DException) {
    ...
    if(m_videoAPI == GP2D_SDL){
        ...
        // Sets up the size of the depth  buffer
...
GP2DRect::GP2DRect(gp2dFloat x_, gp2dFloat y_, gp2dFloat w_, gp2dFloat h_) :
        x(x_),
        y(y_),
        w(w_),
        h(h_) {
...
void GP2DCursor::setEnabled(gp2dBool enabled) {
    m_enabled = enabled;
}

void GP2DCursor::setTex(GP2DTexture* tex) {
    m_tex = tex;
}

void GP2DCursor::draw() {
    // enable texturization
    glEnable(GL_TEXTURE_2D);
    // blending function, used to make alpha parts transparent or some sort.
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    ...
    glAlphaFunc(GL_GREATER, 0.1f);
    glEnable(GL_ALPHA_TEST);
    glBindTexture(GL_TEXTURE_2D, m_tex->getId()); // make sure we use the correct texture ID.
    glDisable(GL_POLYGON_SMOOTH);
    ...
    clog << "Drawn at: " << x1 << ", " << y1 << " - " << x2 << ", " << y2 << endl;
