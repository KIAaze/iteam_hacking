...
namespace gp2d {
	...
	class Sprite {
		public:
			...
			gameTimer	animationTimer;	// Sprites have there own built in timer for animation
		...
		public:
			...
			void Draw(void);								// Draw the whole texture
			void DrawFrame(void);					// Draw the sprite animation frame
			...
			void setAnimationSpeed(GLfloat speed) { this->animationSpeed = speed; }
			GLfloat getAnimationSpeed(void) { return this->animationSpeed; }
			
			void SetWindowing(GLfloat x, GLfloat y, GLfloat w, GLfloat h);
			
//			void ResetAnim();						// Reset the sprite's animation to zero.
//			void AddAnimSequence(int);			// Add an image to the animation sequence.
//			void AddAnimSequence(int, int);	// Same, with a delay as second parameter.
//			void Animate();						// Animate the sprite.
//			void Animate(int);					// Same, with a delay.
//			void Animate(int, int, int);		// Animate from A to B with a delay.
