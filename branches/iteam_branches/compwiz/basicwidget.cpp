...
//		Widget::~Widget();
	...
	void ImageButton::Update() {
		...
		if (isExploding==false) {
			if (isOver) {
				isAnimationOffFinished=false;
				textureOff.currentSheet = 0;
				if (isAnimationOnFinished==false) {
					if (textureOn.setNextFrame() == false) {
						isAnimationOnFinished = true;
						textureOn.currentSheet = 13;
						textureOn.setAnimationSpeed(0.0f);
					}
					else {
				...
				if (isAnimationOffFinished==false) {
					if (textureOff.setNextFrame() == false) {
						isAnimationOffFinished = true;
						textureOff.currentSheet = 13;
						textureOff.setAnimationSpeed(0.0f);
					}
					else {
						textureOff.setAnimationSpeed(28.0f);
					}
				}
			}
		}
