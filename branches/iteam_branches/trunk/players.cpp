	...
	*/
	void AddPlayer ( int PlayerID, GLfloat x, GLfloat y, bool Mirror2, GLfloat Scaling ) {
		...
		if ( PlayerID == IT_PLAYER_SUSI ) {
			Player[Player.size()-1].iterateSheets = false;
			...
			Player[Player.size()-1].Load ( Player_Texture[1], 12, 1 );
			Player[Player.size()-1].selectSheet ( 0 );
			Player[Player.size()-1].ScaleXY ( 0.75,1,1 );
		}
		...
		Player[Player.size()-1].m_FireAngle=iRand ( 0,360 );
