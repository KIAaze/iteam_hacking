...
void AddComponent(int WeaponID, int TypeID, const char texture[], bool IsPhysical, bool Bounces, GLfloat BlastRadius, GLfloat BlastFactor, GLfloat BlastDamage, GLfloat Elasticity, int TriggerTime, bool Active, GLfloat scale)	{
...
int FireWeapon(int TypeID, GLfloat nx, GLfloat ny, GLfloat new_angle, GLfloat new_vel)	{
	...
	for(i=0; i < WeaponsList[TypeID].components.size(); i++)	{
		...
		WeaponsList[TypeID].components[ i ].LastTicks = iTimer.GetTicks(); // used for frame rate limiting the physics code
		WeaponsList[TypeID].components[ i ].StartTicks = iTimer.GetTicks(); // used for timed explosions
...
int FireComponent(int TypeID, int CompID, GLfloat nx, GLfloat ny, GLfloat vel_x, GLfloat vel_y)	{
	...
	WeaponsList[TypeID].components[ CompID ].LastTicks = iTimer.GetTicks(); // used for frame rate limiting the physics code
	WeaponsList[TypeID].components[ CompID ].StartTicks = iTimer.GetTicks(); // used for timed explosions
	return(0);
}
...
void Weapons_Init(void)	{
	// ============================ GRENADES ===========================================
	AddWeaponTypeToWeaponList(THUMBS_WIDTH + IT_WT_GRENADE*34, 600 - THUMBS_HEIGHT, IT_WT_GRENADE, "data/weapons/grenade.png" );
	AddComponent(0, IT_WT_GRENADE, "data/weapons/grenade.png", true, true, (GLfloat) IT_WT_GRENADE_BLAST_RADIUS,IT_WT_GRENADE_BLAST_FACTOR,IT_WT_GRENADE_BLAST_DAMAGE, (GLfloat) 0.80, grenade_time);
	
	// ============================ MISSILES ============================================
	AddWeaponTypeToWeaponList(THUMBS_WIDTH + IT_WT_MISSILE*34, 600 - THUMBS_HEIGHT, IT_WT_MISSILE, "data/weapons/missile.png" );
	...
	// ============================ CLUSTER BOMBS =======================================
	AddWeaponTypeToWeaponList(THUMBS_WIDTH + IT_WT_CLUSTERBOMB*34, 600 - THUMBS_HEIGHT, IT_WT_CLUSTERBOMB, "data/weapons/cluster_bomb.png" );
	...
	// ============================ DYNAMITE ===========================================
	AddWeaponTypeToWeaponList(THUMBS_WIDTH + IT_WT_DYNAMITE*34, 600 - THUMBS_HEIGHT, IT_WT_DYNAMITE, "data/weapons/dynamite.png" );
	...
	// ============================ PISTOL ===========================================
	AddWeaponTypeToWeaponList(THUMBS_WIDTH + IT_WT_PISTOL*34, 600 - THUMBS_HEIGHT, IT_WT_PISTOL, "data/weapons/pistol.png" );
}
...
void DoWeaponBehaviours(int TypeID, int CompID,int collision)	{
	...
	switch(TypeID)	{
		...
		// ==========================================================================================
		case IT_WT_GRENADE:
			newTicks =  iTimer.GetTicks() - WeaponsList[TypeID].components[0].StartTicks;
			if(newTicks > WeaponsList[TypeID].components[0].TimeTrigger)	{  // time trigger reached! go BOOOOOOM!
				...
				sprintf(str, "%d",1+ (WeaponsList[TypeID].components[0].TimeTrigger - newTicks) / 1000 );
				WeaponsList[TypeID].components[0].Name = str;
			}
			break;
		// ==========================================================================================
		// ================================   MISSILES  ==================================================
		...
		case IT_WT_CLUSTERBOMB:
			...
			printf("\n");	
			newTicks =  iTimer.GetTicks() - WeaponsList[TypeID].components[CompID].StartTicks;
			if(newTicks > WeaponsList[TypeID].components[CompID].TimeTrigger)	{  // time trigger reached! go BOOOOOOM!
				...
				sprintf(str, "%d",1+ (WeaponsList[TypeID].components[CompID].TimeTrigger - newTicks) / 1000 );
				WeaponsList[TypeID].components[CompID].Name = str;
			}
			break;
...
void DestroyTerrain(int TypeID, int ComponentID)	{
	...
	GLfloat tempX, tempY;
	SDL_Surface * surf;
	surf = Terrain.animationSheet[0]->spriteTexture.imgF;
	// Find position relative to the image we want to change
	tempX = (Terrain.width[0]/2)-(VIEWPORT_WIDTH/2) + WeaponsList[TypeID].components[ComponentID].x;
	tempY = WeaponsList[TypeID].components[ComponentID].y -  (VIEWPORT_HEIGHT - Terrain.height[0]);
	
	DrawCircleFilled( tempX, tempY, (int)WeaponsList[TypeID].components[ComponentID].BlastRadius, surf, 0x00FFFFFF);
	DrawCircleHalo( tempX, tempY, (int)WeaponsList[TypeID].components[ComponentID].BlastRadius + EXPLOSION_HALO_WIDTH, surf, 0x00FFFFFF);
	Terrain.animationSheet[0]->spriteTexture.Modify(surf);
	//start explosion animation
	TriggerExplosion(WeaponsList[TypeID].components[ComponentID].x , WeaponsList[TypeID].components[ComponentID].y);
}
...
void weapon::CaclulateAcceleration(void)
{
	accel_x=obj_accel_x;
	accel_y=obj_accel_y+Gravity;
	cout<<"after CaclulateAcceleration"<<endl;
...
int weapon::MoveMe(void)	{
	...
	if (CollisionPixel_2(this,&Terrain,0,0,Nx,Ny,Tx,Ty)) {
		x -= advance_x;
		y -= advance_y;
		cout<<"================="<<endl;
