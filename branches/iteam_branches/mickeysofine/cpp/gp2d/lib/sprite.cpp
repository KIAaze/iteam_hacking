...
namespace gp2d {
	...
	{
		GLfloat animDiff;
		animDiff = GetDelta(animationSpeed, animationTimer.GetTicks());
		if (animDiff > 1) {
	...
	int Sprite::Load(const char filename_branch[], int numSpritesWide, int numSpritesHigh) {	// Load a texture as a sprite image...
		...
		img_index = animationSheet.size()-1;
		...
		width.push_back(animationSheet[img_index]->getSpriteWidth());
		height.push_back(animationSheet[img_index]->getSpriteHeight());
