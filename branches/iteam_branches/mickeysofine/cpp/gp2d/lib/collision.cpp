...
*/
		...
		for(px = (Uint32)left_off; px <= (Uint32)right_off;  px++){
            for(py = (Uint32)top_off;  py <= (Uint32)bottom_off; py++){
                ...
                // This line is commented because it caused problems on Windows. It does not affect Linux either, so it's OK to comment it.
                // SDL_GetRGBA(GetPixel(object1->pix[img1],px,py),object1->pix[img1]->format,&pr,&pg,&pb,&pa );	// Get the pixel's alpha from the first image
    ...
    */
    ...
    )
	{
		...
		Nx=0;Ny=0;Tx=0;Ty=0;//default values
...
/* DEBUG INFO: Remove this comment and recompile the library for showing a blue rectangle around each sprite's overlapping rectangle relative to each sprite's image coordinates.*/
    ...
    /* DEBUG INFO: Remove this comment and recompile the library for showing a red area marking the collision area found in the destination sprite.*/
		...
		if(collision)
		{
            ...
            int top_force=0;
            if(over_top == top1)
            { //if top1 border intersects with sprite 2
                py=(Uint32)top_off;
                for(px = (Uint32)left_off; px <= (Uint32)right_off;  px++)
                {
                    SDL_GetRGBA(GetPixel(surface2,px,py),surface2->format,&pr,&pg,&pb,&pa2);
                    if(pa2>0) top_force++;
                }
            }
            ...
            //border 2
            int bottom_force=0;
            if(over_bottom == bottom1)
            { //if bottom1 border intersects with sprite 2
            ...
            //border 3
            int right_force=0;
            if(over_right == right1)
            { //if right1 border intersects with sprite 2
                px=(Uint32)right_off;
                for(py = (Uint32)top_off;  py <= (Uint32)bottom_off; py++)
                {
                    SDL_GetRGBA(GetPixel(surface2,px,py),surface2->format,&pr,&pg,&pb,&pa2);
            ...
            int left_force=0;
            if(over_left == left1)
            { //if left1 border intersects with sprite 2
                px=(Uint32)left_off;
                for(py = (Uint32)top_off;  py <= (Uint32)bottom_off; py++)
                {
                    SDL_GetRGBA(GetPixel(surface2,px,py),surface2->format,&pr,&pg,&pb,&pa2);
