...
//Now we just setup some texture paramaters.
		//if(!glIsTexture(tex_base[ch])) {PrintDebug("FATAL ERROR: tex_base[ch] is not a texture!");exit(13);}
...
//So now we can create the display list
		...
		if ( !glIsTexture ( tex_base[ch] ) ) {printf ( "FATAL ERROR: tex_base[ch] is not a texture!\n" );exit ( 13 );}
		glBindTexture ( GL_TEXTURE_2D,tex_base[ch] );
...
// MODIF: I just changed the function name.
	void Font::Load ( const char * filename_branch, unsigned int h ) {

		char fname[256];
		sprintf ( fname,"%s/%s",DATA_DIR,filename_branch );
		PrintDebug ( "[Font::Load]fname=%s",fname );
...
//        PrintDebug("before i=%d",i);
			make_dlist ( face,i,list_base,textures );
//        PrintDebug("after i=%d",i);
		}
...
// We want a coordinate system where things correspond to window pixels.
...
//for(const char *c=text;*c;c++) {
		...
		glDisable ( GL_DEPTH_TEST );//exit(17);
		glEnable ( GL_BLEND );
