...
#include "gp2d.h"
#include "core.h"
// include fstream to see if files exist
#include "fstream"
// #include "iostream"
...
namespace gp2d {
    ...
    bool CheckLengthPowerOfTwo(int length)
    {
        int i = 0;
        while (i <= sqrt((int)length))
        {
            i++;
            if ((int)pow(2,i) == (int)length)
            {
                return true;
            }
        }
        return false; //is not a power of two
    }
	// ==============================================================================
	// Loads a texture
	// ==============================================================================
	...
	void Texture::Load(const char filename[]){
		...
		if(input.fail())
		{
			...
			char str[256];
			sprintf(str,"File %s does not exist.",filename);
			PrintDebug(str);
			exit(27); //the "missing image" error code
		}
        ...
        if (! CheckLengthPowerOfTwo((int)width) || ! CheckLengthPowerOfTwo((int)height))
        {
            char str[256];
            sprintf(str,"Image dimensions must be powers of two.  Image %s is %i*%i.",filename,(int)width,(int)height);
            PrintDebug(str);
            exit(28); //the "wrong dimensions" error code
        }
