...
{
	printf("FUNCTION CALL: GUI_TermWin:: Display(void)\n");
	int row, i, j;
	Uint8 ch;
	SDL_Rect src;
	SDL_Rect dst;
	...
	printf("row=%d, first_row=%d, scroll_row=%d\n",row,first_row,scroll_row);
	if ( row < 0 ) {
		row = total_rows+row;
		printf("row=%d, total_rows=%d, row=%d\n",row,total_rows,row);
	}
	src.w = charw;
	src.h = charh-1;
	dst.w = charw;
	...
	for ( i=0; i<rows; ++i ) {
		...
		printf("%s\n",str);
	      Text(FNTGameGUI, 10,64+(18+i)*16, 0,1,1,1, 1,1,1, "%s",str);
	}
		FNTGameGUI.Clean();
