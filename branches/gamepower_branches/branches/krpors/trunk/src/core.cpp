...
GP2DException::GP2DException(string what, string detail) :
        m_what(what),
        m_detail(detail),
...
GP2DThread::GP2DThread(string name) :
        m_name(name) {
}
...
gp2dInt32 GP2DThread::registerThread(void* threadPtr) {
    GP2DThread* thread = reinterpret_cast<GP2DThread*>(threadPtr);
    if(thread == NULL) {
...
void GP2DThread::start() throw (GP2DException) {
    ...
    // twice in a GP2DThread object instance. To prevent this, check for the
    // boolean m_running and if the m_thread struct is NOT NULL.
    if(!m_running && m_thread == NULL) {
        m_thread = SDL_CreateThread(GP2DThread::registerThread, this);
        if(m_thread == NULL) {
            ...
            throw GP2DException(err, SDL_GetError());
        }
...
gp2dBool GP2DThread::isRunning() const {
    return m_running;
}

gp2dUint32 GP2DThread::getID() const {
...
void GP2DThread::setName(string name) {
    m_name = name;
}

string GP2DThread::getName() const {
    return m_name;
}
