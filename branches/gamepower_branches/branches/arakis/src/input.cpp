...
void GP2DInputHandler::scanJoysticks(){
	...
	cout << "Scanning joysticks... " << SDL_NumJoysticks() << " found." << endl;
}

int GP2DInputHandler::getNumJoysticks(){
	return SDL_NumJoysticks();
}

GP2DJoyEvent::GP2DJoyEvent(int device, int axis, int value, int button, int state){
	m_joyButton      = button;
	m_joyDevice      = device;
	m_joyAxis        = axis;
...
const int GP2DJoyEvent::getDevice(){
	return m_joyDevice;
}

const int GP2DJoyEvent::getButton(){
	return m_joyButton;
}
const int GP2DJoyEvent::getButtonState(){
	return m_joyButtonState;
}
const int GP2DJoyEvent::getAxis(){
	return m_joyAxis;
}
const int GP2DJoyEvent::getAxisValue(){
	return m_joyAxisValue;
}

void GP2DInputHandler::registerJoyButtonDown(int button, void (*callbackFunction)(void) ) {
    m_mapBasicJoyButtonDown[button] = callbackFunction;
}

void GP2DInputHandler::registerJoyButtonUp(int button, void (*callbackFunction)(void) ) {
    m_mapBasicJoyButtonUp[button] = callbackFunction;
}

void GP2DInputHandler::registerAxisChange(int axis, void (*callbackFunction)(int) ) {
    m_mapBasicJoyAxisChange[axis] = callbackFunction;
}

void GP2DInputHandler::handleFPJoystickInputEvents() {
	int device, button, value, state;
	...
	if(m_event.type == SDL_JOYBUTTONDOWN){
		map<int, void(*)(void)>::iterator it_basic;
		...
		map<int, void(*)(int)>::iterator it_basic;
...
void GP2DInputHandler::registerJoystick(int device, GP2DJoyAction* event) {
...
void GP2DInputHandler::handleOOPJoystickInputEvents() {
	...
	GP2DJoyEvent event(device, axis, value, button, state);
    map<int, GP2DJoyAction*>::iterator it;
