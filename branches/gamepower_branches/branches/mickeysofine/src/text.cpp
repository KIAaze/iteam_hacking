...
TTF_Font* GP2DFont::getFont() const {
    return m_ttf_font;
}
...
void GP2DText::prepareTexture(SDL_Surface* initial) {
    clog << "Preparing texture for use" << endl;
...
void GP2DText::setText(std::string text) {
    ...
    clog << "Rendertype = " << m_renderType << endl;
    switch (m_renderType) {
        case GP2D_RENDER_NORMAL:
            initial = TTF_RenderUTF8_Solid(m_font->getFont(), m_text.c_str(), color);
            break;
        case GP2D_RENDER_NICE:
            // TODO: background color; it's a must have for that function.
            initial = TTF_RenderUTF8_Shaded(m_font->getFont(), m_text.c_str(), color, color);
            break;
        case GP2D_RENDER_NICEST: // fall through the next case
        default:
            initial = TTF_RenderUTF8_Blended(m_font->getFont(), m_text.c_str(), color);
            break;
    }
